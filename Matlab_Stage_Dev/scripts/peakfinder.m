...
% Updated by: Jonas Flueckiger, 2013 
...
if len0 ~= s(1) && len0 ~= s(2)
    ex = MException('PEAKFINDER:Input','The input data must be a vector');
    throw(ex);
elseif isempty(x0)
    varargout = {[],[]};
    return;
...
if ~isreal(x0)
%     ex = MException('PEAKFINDER:NotReal','Absolute value of data will be used');
%     throw(ex);
%     x0 = abs(x0);
    ex = MException('PEAKFINDER:NotReal','Data has complex elements');
    throw(ex);
end
...
if nargin < 3 || isempty(extrema)
    ...
    if extrema == 0
        ex = MException('PEAKFINDER:ZeroMaxima','Either 1 (for maxima) or -1 (for minima) must be input for extrema');
        throw(ex);
    end
end
